...
void tss_set_rsp0(unsigned long long rsp0) { tss.rsp0 = rsp0; }

void gdt_init(void) {
    // Null Descriptor
    gdt.null_desc = (gdt_entry_t){0, 0, 0, 0, 0, 0};

    gdt.kcode = (gdt_entry_t){.limit_low = 0x0000,
                              .base_low = 0x0000,
                              .base_middle = 0x00,
    ...
    gdt.ucode = (gdt_entry_t){.limit_low = 0x0000,
                              .base_low = 0x0000,
                              .base_middle = 0x00,
...
__attribute__((interrupt)) void double_fault_handler(struct interrupt_frame *frame, unsigned long long error_code) {
