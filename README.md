...
- 기본 스레드 스케줄러와 태스크별 페이지 테이블/포트 권한 구조
...
정상적으로 부팅되면 화면에 `XNU OS KERNEL INIT...`와 시스템 콜 호출/복귀 메시지가 출력됩니다.
...
3. 커널 초기화 메시지와 `[TEST] SYSCALL RETURN SUCCESSFUL!` 메시지가 표시되는지 확인합니다.
4. 실패하면 `make`의 컴파일/링커 출력과 QEMU 콘솔 메시지를 함께 확인합니다. 특히 Homebrew 패키지 경로와 OVMF 펌웨어 탐색 여부를 점검합니다.

## 현재 한계와 다음 단계

이 프로젝트는 학습용 최소 구현입니다. 스케줄러의 실제 타이머 인터럽트 연동, 사용자 모드 전환, 안전한 메모리 해제/보호, 실행 파일 로더, 파일 시스템 및 정식 시스템 콜 ABI는 아직 완성되지 않았습니다.

다음 단계로는 APIC 타이머 인터럽트를 스케줄러에 연결해 스레드 전환을 검증하고, 사용자 모드 프로세스와 페이지 권한을 도입하는 것을 권장합니다.
